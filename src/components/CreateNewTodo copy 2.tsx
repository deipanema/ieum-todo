    ...
    try {
      if (isEdit && todoId) {
        ...
        if (todo.goal.id) {
          ...
          await createTodo(todo.title, todo.goal.id, todo.fileUrl || null, todo.linkUrl || null);
        } else {
          // goal이 null인 경우 처리 (예: 오류 메시지 출력)
          toast.error("목표가 지정되지 않았습니다.");
          return;
        }
      }
      ...
      console.log(error);
      // 오류 처리 (예: 오류 메시지 출력)
      toast.error("할 일을 저장하는 중 오류가 발생했습니다.");
    }
  };
